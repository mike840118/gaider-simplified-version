  ...
  <div class="dashboard-wrapper">
    <div class="alert-dropdown-container" v-if="sosPatients.length > 0">
      ...
      <div class="alert-stack" :style="{ zIndex: isAlertExpanded ? 900 : 1 }"
        ...
        <div class="alert-banner" v-for="(patient, index) in (isAlertExpanded ? sosPatients : sosPatients.slice(0, 1))"
    ...
    <div class="panel announcement-panel">
      ...
      <ul class="announce-list">
        <li v-for="item in announcements" :key="item.id" @click="openAnnouncement(item)" style="cursor: pointer;">
    ...
    <div class="middle-section">
      <div class="stats-grid">
        <div class="stat-card" v-for="stat in stats" :key="stat.titleKey">
          ...
          <button v-if="stat.titleKey === 'dashboard.stats.low_battery'" class="btn-view-corner"
            @click="openBattery(stat)">
            {{ $t('dashboard.view') }}
          </button>
        </div>
      </div>
...
const openBattery = (statItem) => {
  // 這裡假設你要開啟的是低電量清單
  // 你可以根據需要傳入篩選好的資料
  const lowBatteryPatients = patients.value.filter(p => p.electricity < 30);
  selectedInfo.value = {
    title: statItem.titleKey,
    data: lowBatteryPatients
  };
